...
function categorizeLine(input) {
    ...
    const isBrandSayDigital = config_1.CATEGORIZATION_CONFIG.BRAND_SAY_DIGITAL_KEYWORDS.some(keyword => channel.includes(keyword) || mediaType.includes(keyword));
    ...
    const isInfluencer = config_1.CATEGORIZATION_CONFIG.INFLUENCER_KEYWORDS.some(keyword => placements.includes(keyword.toLowerCase()) ||
    ...
    const isSocialPlatform = config_1.CATEGORIZATION_CONFIG.SOCIAL_PLATFORMS.some(socialPlatform => platform.includes(socialPlatform.toLowerCase()) || channel.includes(socialPlatform.toLowerCase()));
