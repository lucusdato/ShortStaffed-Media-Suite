...
exports.generateTrafficSheet = generateTrafficSheet;
exports.generateTrafficSheetFromHierarchy = generateTrafficSheetFromHierarchy;
 ...
 */
function categorizeCampaignLine(campaignLine) {
    // Debug logging
    ...
    const result = (0, categorization_1.categorizeLine)({
 ...
 */
function categorizeRow(row, headers) {
    ...
    const isHeaderRow = config_1.CATEGORIZATION_CONFIG.SECTION_HEADER_PATTERNS.some(pattern => channel.toLowerCase() === pattern);
    if (isHeaderRow)
        return { tab: 'section-header', type: channel };
    // Brand Say Digital: Digital Video, Digital Display, etc. (NOT social)
    const isBrandSayDigital = config_1.CATEGORIZATION_CONFIG.BRAND_SAY_DIGITAL_KEYWORDS.some(keyword => channel.includes(keyword));
    if (isBrandSayDigital) {
        return { tab: 'Brand Say Digital', type: 'media' };
    }
    // Check if it's a social platform (Meta, TikTok, Pinterest, etc.)
    const isSocialPlatform = config_1.CATEGORIZATION_CONFIG.SOCIAL_PLATFORMS.some(socialPlatform => channel.includes(socialPlatform) || platform.includes(socialPlatform) || placement.includes(socialPlatform) || tactic.includes(socialPlatform));
    // Brand Say Social: Paid Social OR any social platform (Meta, TikTok, Pinterest, etc.)
    const isPaidSocial = config_1.CATEGORIZATION_CONFIG.PAID_SOCIAL_KEYWORDS.some(keyword => channel.includes(keyword));
    if (isPaidSocial || isSocialPlatform) {
        // Only categorize as Other Say Social if explicitly marked as Influencer
        const isInfluencer = config_1.CATEGORIZATION_CONFIG.INFLUENCER_KEYWORDS.some(keyword => placement.includes(keyword) || tactic.includes(keyword));
        if (isInfluencer) {
 ...
 */
function createColumnMap(tabName, blockingHeaders, trafficSheetHeaderRow) {
    const map = new Map();
    ...
    const baseMappings = config_1.COLUMN_MAPPING_CONFIG.BASE_MAPPINGS;
    const tabSpecificMappings = config_1.COLUMN_MAPPING_CONFIG.TAB_SPECIFIC_MAPPINGS[tabName] || {};
 ...
 */
function captureTemplateBlock(worksheet, startRow, blockSize) {
    const block = {
        rows: [],
        merges: []
    };
    ...
    Object.keys(merges).forEach((mergeAddr) => {
 ...
 */
function applyTemplateBlock(worksheet, templateBlock, targetStartRow, sourceStartRow) {
    // Apply each row
    templateBlock.rows.forEach((rowData, i) => {
        ...
        rowData.cells.forEach((cellData) => {
            ...
            const isHeaderLabel = config_1.STYLE_CONFIG.HEADER_LABELS.includes(cellValue);
            if (isHeaderLabel) {
                ...
                if (config_1.STYLE_CONFIG.TEMPLATE_TEXT_TO_CLEAR.includes(cellValue)) {
 ...
 */
function clearHeaderBorders(worksheet) {
    ...
    const maxColumn = worksheet.name === 'Brand Say Digital'
        ? config_1.TRAFFIC_SHEET_CONFIG.BRAND_SAY_DIGITAL_END_COL
        : config_1.TRAFFIC_SHEET_CONFIG.BRAND_SAY_SOCIAL_END_COL;
    for (let rowNum = config_1.TRAFFIC_SHEET_CONFIG.HEADER_AREA_START_ROW; rowNum <= config_1.TRAFFIC_SHEET_CONFIG.HEADER_AREA_END_ROW; rowNum++) {
 ...
 */
function applyBordersToTacticArea(worksheet, startRow, endRow) {
    ...
    const borderConfig = config_1.TRAFFIC_SHEET_CONFIG.BORDER_CONFIG[worksheet.name] ||
 ...
 */
function forceBordersOnMergedCells(worksheet, startRow, endRow, startCol, endCol) {
 ...
 */
function mergeTacticDataCells(worksheet, startRow, blockSize, columnMap, headerRow) {
    ...
    const columnsToMerge = new Set();
    // First, identify the Accutics Ad Set Name column
    let adSetNameColumn = null;
    ...
    headerRow.eachCell({ includeEmpty: false }, (cell, colNumber) => {
        ...
        if (config_1.STYLE_CONFIG.MERGEABLE_COLUMN_HEADERS.some(h => headerValue.includes(h) || headerValue === h)) {
    ...
    if (adSetNameColumn !== null) {
        ...
        const creativesPerGroup = config_1.TRAFFIC_SHEET_CONFIG.CREATIVES_PER_AD_GROUP;
        const numGroups = config_1.TRAFFIC_SHEET_CONFIG.AD_GROUPS_PER_TACTIC;
 ...
 */
function formatDateForTrafficSheet(dateValue) {
    let date;
    ...
    const month = config_1.DATE_CONFIG.MONTH_NAMES[date.getUTCMonth()];
 ...
 */
function populateTacticHeaderRow(row, tacticData, columnMap) {
 ...
 */
async function generateTrafficSheet(blockingChartData, templateBuffer, manualOverrides = {}) {
    ...
    const isValidTactic = (row) => {
        ...
        const isSummaryRow = config_1.CATEGORIZATION_CONFIG.SUMMARY_ROW_PATTERNS.some(pattern => channelValue.includes(pattern)) || (channelValue.includes('total') && !channelValue.includes('working total'));
        if (isSummaryRow) {
            return false;
        }
        ...
        return keysWithValues.length >= config_1.PARSING_CONFIG.MIN_CAMPAIGN_LINE_FIELDS;
    };
    ...
    blockingChartData.rows.forEach((row, index) => {
        const mergeSpan = row._mergeSpan;
    ...
    for (const tabName of tabNames) {
        ...
        const headerLabelRow = worksheet.getRow(config_1.TRAFFIC_SHEET_CONFIG.HEADER_LABEL_ROW);
        ...
        const templateStartRow = config_1.TRAFFIC_SHEET_CONFIG.TEMPLATE_START_ROW;
        const blockSize = config_1.TRAFFIC_SHEET_CONFIG.CREATIVE_LINES_PER_TACTIC; // Total rows: 15 creative lines (no blank separator between tactics)
        ...
        let currentRow = config_1.TRAFFIC_SHEET_CONFIG.FIRST_DATA_ROW; // Start with first tactic data at row 9
        // Process each tactic
        tactics.forEach((tacticData, tacticIndex) => {
            ...
            const mergeSpan = tacticData._mergeSpan || 1;
            ...
            mergeTacticDataCells(worksheet, currentRow, config_1.TRAFFIC_SHEET_CONFIG.CREATIVE_LINES_PER_TACTIC, columnMap, headerLabelRow);
        ...
        if (tactics.length > 0) {
            console.log(`Applying borders to tactic area: rows ${config_1.TRAFFIC_SHEET_CONFIG.HEADER_LABEL_ROW} to ${currentRow - 1}`);
            applyBordersToTacticArea(worksheet, config_1.TRAFFIC_SHEET_CONFIG.HEADER_LABEL_ROW, currentRow - 1);
        }
 ...
 */
async function generateTrafficSheetFromHierarchy(blockingChartData, templateBuffer, manualOverrides) {
    const { writeCampaignLineToWorksheet, buildColumnMap } = await Promise.resolve().then(() => __importStar(require('./trafficSheetWriter')));
    ...
    blockingChartData.campaignLines.forEach((campaignLine, index) => {
        ...
        if (tab in campaignLinesByTab) {
            campaignLinesByTab[tab].push(campaignLine);
        }
        else {
    ...
    for (const tabName of tabNames) {
        ...
        const headerRow = config_1.TRAFFIC_SHEET_CONFIG.HEADER_LABEL_ROW;
        ...
        campaignLines.forEach((campaignLine, index) => {
            ...
            const result = writeCampaignLineToWorksheet(worksheet, campaignLine, currentRow, tabName, columnMap, []);
        ...
        const dataStartRow = config_1.TRAFFIC_SHEET_CONFIG.FIRST_DATA_ROW; // Row 9 for all tabs
        ...
        try {
            ...
            if (h11.fill) {
                ...
                console.log(`    - Fill pattern: ${h11.fill.pattern || 'N/A'}`);
            }
        }
        catch (err) {
        ...
        try {
            ...
            if (o11.fill) {
                ...
                console.log(`    - Fill pattern: ${o11.fill.pattern || 'N/A'}`);
            }
        }
        catch (err) {
        ...
        try {
            ...
            if (p11.fill) {
                ...
                console.log(`    - Fill pattern: ${p11.fill.pattern || 'N/A'}`);
            }
        }
        catch (err) {
    ...
    if (brandSayDigitalSheet) {
        ...
        forceBordersOnMergedCells(brandSayDigitalSheet, 9, // Start row (first data row)
