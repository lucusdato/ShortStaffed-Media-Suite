...
exports.formatValidationErrors = exports.validateBlockingChart = void 0;
exports.parseBlockingChart = parseBlockingChart;
exports.validateBlockingChartLegacy = validateBlockingChartLegacy;
 ...
 */
function getAdGroupCount(platform, mediaType, hasBothDisplayAndVideo) {
    const platformLower = (platform || '').toLowerCase();
    const mediaTypeLower = (mediaType || '').toLowerCase();
    // Check if programmatic platform
    const isProgrammatic = config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.PROGRAMMATIC.PLATFORMS.some(p => platformLower.includes(p.toLowerCase()));
    if (isProgrammatic) {
        const isDisplay = mediaTypeLower.includes('display');
        const isVideo = mediaTypeLower.includes('video');
        if (hasBothDisplayAndVideo) {
            if (isDisplay)
                return config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.PROGRAMMATIC.DISPLAY_WITH_VIDEO;
            if (isVideo)
                return config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.PROGRAMMATIC.VIDEO_WITH_DISPLAY;
        }
        else {
            // Only display or only video exists in blocking chart
            if (isDisplay)
                return config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.PROGRAMMATIC.DISPLAY_ONLY;
            if (isVideo)
                return config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.PROGRAMMATIC.VIDEO_ONLY;
        }
    }
    // Check if Meta
    const isMeta = config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.META.PLATFORMS.some(p => platformLower.includes(p.toLowerCase()));
    if (isMeta)
        return config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.META.AD_GROUPS;
    // Check if other social
    const isOtherSocial = config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.OTHER_SOCIAL.PLATFORMS.some(p => platformLower.includes(p.toLowerCase()));
    if (isOtherSocial)
        return config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.OTHER_SOCIAL.AD_GROUPS;
    // Check if non-programmatic display or audio
    const isDisplayAudio = config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.DISPLAY_AUDIO.MEDIA_TYPES.some(mt => mediaTypeLower.includes(mt.toLowerCase()));
    if (isDisplayAudio && !isProgrammatic) {
        return config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.DISPLAY_AUDIO.AD_GROUPS;
    }
    // Default
    return config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.DEFAULT.AD_GROUPS;
}
/**
 ...
 */
function detectExcludedChannel(channel, platform, placements, adFormat) {
    ...
    const excludedCategories = config_1.CATEGORIZATION_CONFIG.EXCLUDED_CHANNEL_KEYWORDS;
    // Check OOH
 ...
 */
function hasBothProgrammaticDisplayAndVideo(rows) {
    ...
    rows.forEach(row => {
        ...
        const isProgrammatic = config_1.ROW_EXPANSION_CONFIG.AD_GROUP_RULES.PROGRAMMATIC.PLATFORMS.some(p => platformLower.includes(p.toLowerCase()));
        if (isProgrammatic) {
            if (mediaTypeLower.includes('display'))
                hasDisplay = true;
            if (mediaTypeLower.includes('video'))
                hasVideo = true;
        }
    });
 ...
 */
function buildCampaignLines(rows, campaignLineMerges) {
    const campaignLines = [];
    ...
    const rowsByCampaignLine = new Map();
    rows.forEach(row => {
    ...
    rowsByCampaignLine.forEach((campaignRows, masterRow) => {
        ...
        const adGroupCount = getAdGroupCount(platform, mediaType, hasBothProgDisplayVideo);
        // Build ad groups
        const adGroups = [];
        for (let i = 0; i < adGroupCount; i++) {
            const creativeLines = [];
            // Create 5 creative lines per ad group
            for (let j = 0; j < config_1.ROW_EXPANSION_CONFIG.CREATIVES_PER_AD_GROUP; j++) {
                creativeLines.push({
        ...
        const totalRows = adGroupCount * config_1.ROW_EXPANSION_CONFIG.CREATIVES_PER_AD_GROUP;
 ...
 */
function detectCampaignLineMerges(worksheet, budgetColIndex, impressionsColIndex, placementsColIndex, headerRowIndex, headers) {
    const campaignLineMerges = new Map();
    ...
    const isTotalOrHeaderRow = (row) => {
        ...
        const isTotal = totalPatterns.some(pattern => channelValue.includes(pattern) ||
        ...
        const isSectionHeader = sectionHeaderPatterns.some(pattern => channelValue === pattern || platformValue === pattern);
    ...
    const budgetMerges = [];
    const impressionsMerges = [];
    const placementsMerges = [];
    ...
    Object.keys(merges).forEach(key => {
        ...
        const colIndex = startCol.split('').reduce((acc, char) => acc * 26 + char.charCodeAt(0) - 65, 0);
    ...
    const uniqueImpressionsMerges = Array.from(new Map(impressionsMerges.map(im => [im.startRow, im])).values());
    ...
    const uniquePlacementsMerges = Array.from(new Map(placementsMerges.map(pm => [pm.startRow, pm])).values());
    ...
    budgetMerges.forEach(budgetMerge => {
        const matchingImpressionsMerge = impressionsMerges.find(im => im.startRow === budgetMerge.startRow && im.span === budgetMerge.span);
        const matchingPlacementsMerge = placementsMerges.find(pm => pm.startRow === budgetMerge.startRow && pm.span === budgetMerge.span);
    ...
    worksheet.eachRow((row, rowNumber) => {
        if (rowNumber <= headerRowIndex)
            return;
        if (hitTotalRow)
            return; // Stop processing after first total row
        const budgetCell = row.getCell(budgetColIndex + 1);
        const impressionsCell = row.getCell(impressionsColIndex + 1);
        const placementsCell = row.getCell(placementsColIndex + 1);
        const budgetValue = getCellValue(budgetCell);
        ...
        if (rowNumber === 48 || rowNumber === 49 || rowNumber === 57) {
            ...
            const isPartOfMerge = Array.from(campaignLineMerges.entries()).some(([masterRow, span]) => rowNumber >= masterRow && rowNumber < masterRow + span);
            console.log(`   Part of existing merge? ${isPartOfMerge}`);
            if (isPartOfMerge) {
                const mergeInfo = Array.from(campaignLineMerges.entries()).find(([masterRow, span]) => rowNumber >= masterRow && rowNumber < masterRow + span);
                if (mergeInfo) {
            ...
            if (budgetValue && impressionsValue && placementsValue) {
                ...
                if (!impressionsValue)
                    console.log(`     - Missing impressions`);
                if (!placementsValue)
                    console.log(`     - Missing placements`);
            }
            console.log('');
        }
        ...
        if (budgetValue && impressionsValue && placementsValue && !campaignLineMerges.has(rowNumber)) {
            const isPartOfExistingMerge = Array.from(campaignLineMerges.entries()).some(([masterRow, span]) => rowNumber >= masterRow && rowNumber < masterRow + span);
 ...
 */
async function parseBlockingChart(fileBuffer, tabIndex) {
    ...
    worksheet.eachRow((row, rowNumber) => {
        ...
        if (nonEmptyCount >= config_1.PARSING_CONFIG.MIN_HEADER_CELLS) {
            ...
            const hasRequiredKeywords = config_1.PARSING_CONFIG.REQUIRED_HEADER_KEYWORDS.every(keyword => normalizedHeaders.some(h => h.includes(keyword)));
    ...
    const budgetColumnIndex = headers.findIndex(h => {
        ...
        return config_1.PARSING_CONFIG.BUDGET_COLUMN_NAMES.some(budgetName => trimmed === budgetName);
    });
    // Detect the impressions column for campaign line detection
    const impressionsColumnIndex = headers.findIndex(h => {
        const trimmed = h.trim();
        return config_1.PARSING_CONFIG.IMPRESSIONS_COLUMN_NAMES.some(impName => trimmed === impName);
    });
    // Detect the placements column for campaign line detection (with fallback)
    const placementsColumnIndex = headers.findIndex(h => {
        const trimmed = h.trim();
        return config_1.PARSING_CONFIG.PLACEMENTS_COLUMN_NAMES.some(placementName => trimmed === placementName);
    });
    if (budgetColumnIndex === -1) {
        throw new Error(`Budget column not found. Expected one of: ${config_1.PARSING_CONFIG.BUDGET_COLUMN_NAMES.join(', ')}`);
    }
    if (impressionsColumnIndex === -1) {
        throw new Error(`Impressions column not found. Expected one of: ${config_1.PARSING_CONFIG.IMPRESSIONS_COLUMN_NAMES.join(', ')}`);
    }
    if (placementsColumnIndex === -1) {
        throw new Error(`Placements column not found. Expected one of: ${config_1.PARSING_CONFIG.PLACEMENTS_COLUMN_NAMES.join(', ')}`);
    }
    ...
    const campaignLineMerges = detectCampaignLineMerges(worksheet, budgetColumnIndex, impressionsColumnIndex, placementsColumnIndex, headerRowIndex, headers);
    ...
    worksheet.eachRow((row, rowNumber) => {
        ...
        row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
            ...
            if (value !== null && value !== undefined && value !== "") {
                ...
                const fieldName = config_1.UNIFIED_TEMPLATE_CONFIG.FIELD_MAPPINGS[header]
                    || normalizeHeaderName(header);
                rowData[fieldName] = value;
            }
        });
        // Store merge span from campaign line detection
        const campaignLineMerge = Array.from(campaignLineMerges.entries()).find(([masterRow, span]) => rowNumber >= masterRow && rowNumber < masterRow + span);
    ...
    const validation = (0, validation_1.validateBlockingChart)(result);
    if (!validation.valid || validation.warnings.length > 0) {
        console.log('\n⚠️  VALIDATION RESULTS:');
        console.log((0, validation_1.formatValidationErrors)(validation));
        console.log('');
    }
    else {
...
function getCellValue(cell) {
    ...
    const isDateType = cell.type === exceljs_1.default.ValueType.Date;
 ...
 */
function extractMetadata(worksheet, headerRowIndex) {
    const metadata = {};
    // Look at the first few rows before the header (typically rows 3-10 in new template)
    for (let i = 1; i < headerRowIndex && i <= config_1.PARSING_CONFIG.MAX_METADATA_ROWS; i++) {
 ...
 */
function validateBlockingChartLegacy(parsed) {
    const errors = [];
    ...
    const hasCommonHeaders = config_1.PARSING_CONFIG.COMMON_HEADERS.some(ch => normalizedHeaders.some(nh => nh.includes(ch)));
    if (!hasCommonHeaders) {
...
Object.defineProperty(exports, "validateBlockingChart", { enumerable: true, get: function () { return validation_2.validateBlockingChart; } });
Object.defineProperty(exports, "formatValidationErrors", { enumerable: true, get: function () { return validation_2.formatValidationErrors; } });
