...
import { CampaignLine, AdGroup, CreativeLine } from './types';
import { ROW_EXPANSION_CONFIG, DATE_CONFIG, TRAFFIC_SHEET_CONFIG } from './config';
...
/**
 * Merge information to be applied later
 */
 ...
 */
export function writeCampaignLineToWorksheet(
  ...
  merges: MergeInfo[] = []
): { nextRow: number; merges: MergeInfo[] } {
    ...
    Object.entries(adGroupFields).forEach(([fieldName, value]) => {
      ...
      if (colNumber) {
        ...
        merges.push({
          ...
          startCol: colNumber,
          endRow: adGroupStartRow + ROW_EXPANSION_CONFIG.CREATIVES_PER_AD_GROUP - 1,
          endCol: colNumber
        });
        ...
        console.log(`   📌 Tracked merge for "${fieldName}" across rows ${adGroupStartRow}-${adGroupStartRow + ROW_EXPANSION_CONFIG.CREATIVES_PER_AD_GROUP - 1}`);
      } else {
        console.log(`   ⚠️  Column not found for field "${fieldName}"`);
      }
    });
 ...
 */
function getCampaignLevelFields(campaignLine: CampaignLine, tabName: string): Record<string, any> {
  const demo = extractDemographic(campaignLine.target);
 ...
 */
function getAdGroupLevelFields(adGroup: AdGroup, campaignLine: CampaignLine, tabName: string): Record<string, any> {
 ...
 */
function getCreativeLevelFields(creativeLine: CreativeLine, creativeNumber: number): Record<string, any> {
